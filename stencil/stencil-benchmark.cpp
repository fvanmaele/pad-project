...
void initData(const int Nx, const int Ny, const int Nz, const int radius, float* A, float* B,
              float* vsq) {
    int offset = 0;
    for (int z = 0; z < Nz; ++z)
        for (int y = 0; y < Ny; ++y)
...
void domainTile(std::vector<benchParam>* benchmark, const int x, const int y,
                const int z) {
...
int main(int argc, char** argv) {
    ...
    for (auto& state : benchmark) {
        ...
        std::vector<float> coeff(radius);
        double time = 0;
        //init coefficients
        for (auto& c : coeff)
            c = 0.1f;

        initData(outerX, outerY, outerZ, radius, Veven.data(), Vodd.data(),
                 Vsq.data());
        ...
        printCSV(state.x, state.y, state.z, state.xtile, state.ytile,
                 state.ztile, time, bw);
    }
    return 0;
}
