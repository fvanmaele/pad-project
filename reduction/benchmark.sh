...
((run_upcxx_skl)) && { 
    ...
    for i in {15..30}; do
        srun -w mp-media1 upcxx-run -n 4 -shared-heap 80% \
            reduction/reduction-upcxx-skl --size "$i" --iterations "$iterations"
    done
} > reduction-dist-skl-upcxx.csv

# KNL, UPCXX (64 processes)
((run_upcxx_knl)) && {
    printf 'X,Time[s],Throughput[GB/s]\n'
    for i in {15..30}; do
        srun -w mp-knl1 upcxx-run -n 64 -shared-heap 80% \
            reduction/reduction-upcxx-knl --size "$i" --iterations "$iterations"
    done 
...
((run_openmp_skl)) && {
    ...
    for i in {15..30}; do
        srun -w mp-media1 upcxx-run -n 1 -shared-heap 80% \
            env OMP_NUM_THREADS=4 reduction/reduction-upcxx-openmp-skl --size "$i" --iterations "$iterations"
    done 
...
((run_openmp_knl)) && {
    ...
    for i in {15..30}; do
        srun -w mp-knl1 upcxx-run -n 1 -shared-heap 80% \
            env OMP_NUM_THREADS=64 reduction/reduction-upcxx-openmp-knl --size "$i" --iterations "$iterations"
    done
...
((run_upcxx_knl_dist)) && {
    printf 'X,Time[s],Throughput[GB/s]\n'
    for i in {15..30}; do
        GASNET_SPAWNFN=C GASNET_CSPAWN_CMD="srun -w mp-knl1 -n %N %C" upcxx-run -N 4 -n 256 -shared-heap 80% \
            reduction/reduction-upcxx-knl --size "$i" --iterations "$iterations"
    done
} > reduction-dist-knl-upcxx.csv
...
((run_openmp_skl_dist)) && {
    printf 'X,Time[s],Throughput[GB/s]\n'
    for i in {15..30}; do
        GASNET_SPAWNFN=C GASNET_CSPAWN_CMD="srun -w mp-media1 -n %N %C" upcxx-run -N 4 -n 4 -shared-heap 80% \
            env OMP_NUM_THREADS=4 reduction/reduction-upcxx-openmp-skl --size "$i" --iterations "$iterations"
    done
} > reduction-dist-skl-upcxx-openmp.csv
...
((run_openmp_knl_dist)) && {
    printf 'X,Time[s],Throughput[GB/s]\n'
    for i in {15..30}; do
        GASNET_SPAWNFN=C GASNET_CSPAWN_CMD="srun -w mp-knl1 -n %N %C" upcxx-run -N 4 -n 4 -shared-heap 80% \
            env OMP_NUM_THREADS=64 reduction/reduction-upcxx-openmp-knl --size "$i" --iterations "$iterations"
    done
} > reduction-dist-knl-upcxx-openmp.csv
